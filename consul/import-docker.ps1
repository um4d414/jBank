...
param(
  ...
  [string]$EnvName = "docker",
  [string]$RootDir = ".\configs",
  [int]$WaitTimeout = 60,
  [int]$WaitInterval = 2
)

Write-Host "Import (docker) $RootDir\$EnvName -> $ConsulAddr/v1/kv/config/{service}/data"
...
$services = @()
# top-level files
Get-ChildItem -Path $envPath -File -ErrorAction SilentlyContinue | ForEach-Object {
  if ($_.Name -match '^(.+)\.(ya?ml|yaml|properties)$') { $svc = $Matches[1]; if (-not ($services -contains $svc)) { $services += $svc } }
}
# directories
Get-ChildItem -Path $envPath -Directory -ErrorAction SilentlyContinue | ForEach-Object { if (-not ($services -contains $_.Name)) { $services += $_.Name } }
...
foreach ($svc in $services) {
  ...
  try {
    if (Test-Path $topYml) {
      ...
      Write-Warning "No source files for service $svc. Skipping."
...
Write-Host "Import (docker) finished."