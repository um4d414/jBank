...
# language: bash
# import-local.sh
# Импортирует ./configs/local -> Consul keys: config/{service}/data
set -e

CONSUL_ADDR="${CONSUL_ADDR:-http://localhost:8500}"
ENV_NAME="local"
ROOT_DIR="${ROOT_DIR:-./configs}"
WAIT_TIMEOUT="${CONSUL_WAIT_TIMEOUT:-30}"
WAIT_INTERVAL="${CONSUL_WAIT_INTERVAL:-2}"

echo "Import (local) ${ROOT_DIR}/${ENV_NAME} -> ${CONSUL_ADDR}/v1/kv/config/{service}/data"
...
# find services either as top-level files or directories
